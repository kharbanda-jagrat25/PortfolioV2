        ...
        <div className="hidden md:flex justify-end items-center">
          <ul className="flex flex-row space-x-8">
            {navBarItems.map(({ label, id }, index) => (
              <li
                key={label}
                className={`flex justify-center pb-1 hover:text-teal-500 transition-all duration-500 text-sm font-light cursor-pointer relative text-white group ${
                  index === navBarItems.length - 1 ? "pr-6" : ""
                }`}
                onClick={() => scrollItemToView(id)}
              >
                {label}
                <span className="absolute bottom-0 left-0 h-0.5 bg-teal-500 w-0 group-hover:w-full transition-all duration-500"></span>
              </li>
            ))}
          </ul>
          <PrimaryButton
